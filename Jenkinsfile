  ...
  stages {
    ...
    stage('Build mysql image') {
      ...
      steps {
        ...
        '''.stripIndent())
        sh 'docker build -t $DOCKER_REGISTRY/entropypool/mysql:5.7.35 .'
      }
    }
    ...
    stage('Release mysql image') {
      ...
      steps {
        sh(returnStdout: true, script: '''
          set +e
          while true; do
            docker push $DOCKER_REGISTRY/entropypool/mysql:5.7.35
            if [ $? -eq 0 ]; then
              break
            fi
          done
    ...
    stage('Execute base sql') {
      ...
      steps {
        sh (returnStdout: false, script: '''
            ...
            kubectl exec -it -n kube-system mysql-0 -- mysql -uroot -p$PASSWORD -e "`cat ./sql/base.sql`"
            kubectl create secret generic mysql-exporter-password-secret --from-literal=password=$MYSQL_EXPORTER_PASSWORD -n monitor
        '''.stripIndent())
      }
    }
